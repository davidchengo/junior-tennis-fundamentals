(async function () {
  ...
  function render(report) {
    ...
    document.getElementById('playerSnapshot').innerHTML = `<article class="stat-card"><span>Ball stage</span><strong>${app.escapeHtml(title(player.current_ball_stage || 'Not set'))}</strong></article><article class="stat-card"><span>Tennis start</span><strong>${app.formatDate(player.tennis_start_date)}</strong></article><article class="stat-card"><span>Rally School start</span><strong>${app.formatDate(player.rally_school_start_date)}</strong></article>`;
    ...
    document.getElementById('reportAssessments').innerHTML = assessments.length ? assessments.map(item => `<article class="list-item"><header><strong>${app.escapeHtml(item.skill)}</strong><div class="tag-list"><span class="tag">${app.escapeHtml(item.difficulty_context)}</span>${item.supersedes_assessment_id ? '<span class="tag">Correction</span>' : ''}${superseded.has(item.id) ? '<span class="tag">Superseded</span>' : ''}</div></header><p class="muted">${app.formatDate(item.evaluated_at)}</p>${item.correction_reason ? `<p><strong>Correction reason:</strong> ${app.escapeHtml(item.correction_reason)}</p>` : ''}${scoreTags(item)}${objectiveText(item)}${item.note ? `<p>${app.escapeHtml(item.note)}</p>` : ''}</article>`).join('') : '<p class="muted">No assessments were recorded when this report was published.</p>';
  }
